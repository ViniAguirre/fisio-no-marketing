...
1. Read `.qwen/commands/add.plan.md` — PRIMARY reference.
   Execute as if `--yolo` (skip [STOP] points, no confirmations).
2. Read `.qwen/skills/add-tasks-checklist/SKILL.md` — canonical tasks.md schema and architect prompt template.
...
1. Read `.qwen/commands/add.build.md` — reference for patterns and conventions.
...
1. Read `.qwen/commands/add.review.md` — PRIMARY reference.
   Execute as if `--yolo` (skip [STOP] points, no confirmations).
2. Run: `bash .codeadd/scripts/status.sh`
...
Any mutation to `plan.md` or `about.md` executed by dispatched agents MUST obey the **cache documental** rule from `.qwen/skills/add-doc-schemas/SKILL.md`:
...
For EACH mutated doc (`plan.md` and/or `about.md`), execute the validation gate from `.qwen/skills/add-doc-schemas/SKILL.md` for the corresponding schema (`feature-plan` or `feature-about`). Additionally verify immutable fields (`id:`, `type:`, `created:`) were preserved from pre-mutation.
...
The coordinator (NOT area validators) is the sole writer of `## Validation Gates` ticks in autopilot. Run the **Validation Gates Procedure** from `.qwen/skills/add-tasks-checklist/SKILL.md`:
