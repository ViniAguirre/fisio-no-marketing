...
**Schema load (MANDATORY).** EXECUTE schema `feature-plan` from `.qwen/skills/add-doc-schemas/SKILL.md`. Reuse `[NNNN]F` from about.md. Apply cache technique per `.qwen/skills/add-doc-schemas/SKILL.md`.
...
**MANDATORY:** Load `.qwen/skills/add-tasks-checklist/SKILL.md` BEFORE dispatching. The skill defines the canonical schema, tick rules, failure-marker semantics, and the architect prompt template.
...
Execute the validation gate from `.qwen/skills/add-doc-schemas/SKILL.md` for schema `feature-plan`.
