...
Load `.qwen/skills/add-doc-schemas/SKILL.md` before STEP 1 (schemas, IDs, universal doc rules). Apply `.qwen/skills/add-id-convention/SKILL.md` for ID/branch format.
...
IF SCHEMA NOT LOADED (before mutating plan.md/about.md):
  ⛔ DO NOT USE: Write on plan.md or about.md
  ✅ DO: Load feature-plan / feature-about from .qwen/skills/add-doc-schemas/SKILL.md
...
**MANDATORY:** Load `.qwen/skills/add-tasks-checklist/SKILL.md` BEFORE entering this mode. The skill defines the 5-section schema, tick rules, `[!]` semantics, "non-trivial change" rule, the Resume/Rerun procedure, the tick-application procedure, and the validator report shape.
...
**MANDATORY:** Validator MUST load `.qwen/skills/add-tasks-checklist/SKILL.md` to apply tick rules, "non-trivial change" definition, and `[!]` failure-marker semantics.
...
After ALL area validators return AND build verification passes, run the **Validation Gates Procedure** from `.qwen/skills/add-tasks-checklist/SKILL.md`. This performs the final write to `tasks.md` (§5 ticks + final §1 recompute).
...
Whenever implementation status, tasks, or decisions require updating `plan.md` or `about.md`, apply the **cache documental** rule from `.qwen/skills/add-doc-schemas/SKILL.md`:
...
For EACH doc mutated (`plan.md` and/or `about.md`), execute the validation gate from `.qwen/skills/add-doc-schemas/SKILL.md` for the corresponding schema (`feature-plan` or `feature-about`). Additionally verify immutable fields (`id:`, `type:`, `created:`) were preserved from pre-mutation.
