...
- Command/skill/script path references — use `{{cmd:}}` / `{{skill:}}` per `.kilocode/skills/add-resource-path-convention/SKILL.md`
...
- **JSON = DATA. Markdown = INSTRUCTIONS.** Minified JSON is exclusively for structured data the agent looks up (configs, tech stack, path mappings). Rules, instructions, orientation, constraints, and behavioral guidance MUST use markdown (lists, tables, prose). Never put rules inside JSON objects. See `.kilocode/skills/add-token-efficiency/SKILL.md` for detailed examples.
...
> **Convention:** Feature/hotfix/refactor/chore/docs IDs use `[NNNN][L]` (number first, letter suffix) per `.kilocode/skills/add-id-convention/SKILL.md`. `PRD` and `CHG` are separate namespaces with no letter suffix.
...
- **Refs** — doc→doc links MUST use `{{doc:<ID>}}` syntax per the ID Convention section. Command/skill refs use `{{cmd:}}` / `{{skill:}}` per `.kilocode/skills/add-resource-path-convention/SKILL.md`.
...
- `.kilocode/skills/add-token-efficiency/SKILL.md` — generic compression patterns for authoring commands/skills
- `.kilocode/skills/add-resource-path-convention/SKILL.md` — build-time resource refs (`{{cmd:}}` / `{{skill:}}`)
- `.kilocode/skills/add-claude-md-style/SKILL.md` — CLAUDE.md generation rules (separate pipeline)
