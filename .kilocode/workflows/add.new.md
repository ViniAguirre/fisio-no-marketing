...
**MANDATORY GATE:** The `feature-about` schema must be loaded from `.kilocode/skills/add-doc-schemas/SKILL.md` BEFORE writing any doc.
...
> **Skill:** Apply `.kilocode/skills/add-id-convention/SKILL.md` for ID/branch format.
...
2. **DISPATCH AGENT: @discovery-agent — Codebase Discovery**
   ...
   - **Steps:**
     ...
     - Include "Related Features" section in discovery.md (table + ``)
...
EXECUTE schema `feature-about` from `.kilocode/skills/add-doc-schemas/SKILL.md`.

**Apply cache technique:** Read → Preserve → Complement → Metadata (per `.kilocode/skills/add-doc-schemas/SKILL.md`)
...
Execute the validation gate from `.kilocode/skills/add-doc-schemas/SKILL.md` for schema `feature-about`.
...
The agent MUST NOT see this conversation — its job is to read only the doc and the schema. If the provider does not support subagent dispatch, apply `.kilocode/skills/add-doc-reviewer/SKILL.md` inline, explicitly forgetting the conversation.
