...
> **Skill:** Apply `.kilocode/skills/add-id-convention/SKILL.md` for ID/branch format. Changelog IDs use `CHG[NNNN]` (no letter suffix — different namespace).
...
**Schema load (MANDATORY).** EXECUTE schema `changelog` from `.kilocode/skills/add-doc-schemas/SKILL.md`.
...
Execute the validation gate from `.kilocode/skills/add-doc-schemas/SKILL.md` for schema `changelog`.
...
1. Read `.kilocode/workflows/add.pr.md` as reference for PR creation flow
