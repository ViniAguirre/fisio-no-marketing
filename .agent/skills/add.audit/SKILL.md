...
**IF a finding lacks a clear root cause** (symptom observable but cause not isolated, OR severity assessment is uncertain, OR finding crosses pillars): LOAD .agent/skills/add-investigation/SKILL.md and apply Phase 3 (Differential Diagnosis) before finalizing severity classification.
...
**Schema load (MANDATORY).** EXECUTE schema `audit-report` from `.agent/skills/add-doc-schemas/SKILL.md`. Apply cache technique per `.agent/skills/add-doc-schemas/SKILL.md`.
...
Execute the validation gate from `.agent/skills/add-doc-schemas/SKILL.md` for schema `audit-report`.
