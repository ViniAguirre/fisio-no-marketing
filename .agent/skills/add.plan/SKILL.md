...
**Schema load (MANDATORY).** EXECUTE schema `feature-plan` from `.agent/skills/add-doc-schemas/SKILL.md`. Reuse `[NNNN]F` from about.md. Apply cache technique per `.agent/skills/add-doc-schemas/SKILL.md`.
...
**MANDATORY:** Load `.agent/skills/add-tasks-checklist/SKILL.md` BEFORE dispatching. The skill defines the canonical schema, tick rules, failure-marker semantics, and the architect prompt template.
...
Execute the validation gate from `.agent/skills/add-doc-schemas/SKILL.md` for schema `feature-plan`.
