...
> **Skill:** Apply `.agent/skills/add-id-convention/SKILL.md` for ID/branch format. Changelog IDs use `CHG[NNNN]` (no letter suffix — different namespace).
...
**Schema load (MANDATORY).** EXECUTE schema `changelog` from `.agent/skills/add-doc-schemas/SKILL.md`.
...
Execute the validation gate from `.agent/skills/add-doc-schemas/SKILL.md` for schema `changelog`.
...
1. Read `.agent/skills/add.pr/SKILL.md` as reference for PR creation flow
