...
2. Read `.agent/skills/add-tasks-checklist/SKILL.md` — canonical tasks.md schema and architect prompt template.
...
1. Read `.agent/skills/add.build/SKILL.md` — reference for patterns and conventions.
...
1. Read `.agent/skills/add.review/SKILL.md` — PRIMARY reference.
   Execute as if `--yolo` (skip [STOP] points, no confirmations).
2. Run: `bash .codeadd/scripts/status.sh`
...
Any mutation to `plan.md` or `about.md` executed by dispatched agents MUST obey the **cache documental** rule from `.agent/skills/add-doc-schemas/SKILL.md`:
...
For EACH mutated doc (`plan.md` and/or `about.md`), execute the validation gate from `.agent/skills/add-doc-schemas/SKILL.md` for the corresponding schema (`feature-plan` or `feature-about`). Additionally verify immutable fields (`id:`, `type:`, `created:`) were preserved from pre-mutation.
...
The coordinator (NOT area validators) is the sole writer of `## Validation Gates` ticks in autopilot. Run the **Validation Gates Procedure** from `.agent/skills/add-tasks-checklist/SKILL.md`:
