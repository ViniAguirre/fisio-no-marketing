...
IF SCHEMA NOT LOADED:
  ...
  ✅ DO: Load `hotfix-about` and `hotfix-related` sections from .agent/skills/add-doc-schemas/SKILL.md FIRST
...
LOAD .agent/skills/add-investigation/SKILL.md and apply Phases 1-3 (Root Cause Investigation, Pattern Analysis, Differential Diagnosis) before proposing a root cause in STEP 7.
...
EXECUTE schema `hotfix-about` from `.agent/skills/add-doc-schemas/SKILL.md`.
...
EXECUTE schema `hotfix-related` from `.agent/skills/add-doc-schemas/SKILL.md`.
...
Execute the validation gate from `.agent/skills/add-doc-schemas/SKILL.md` for each doc written:
