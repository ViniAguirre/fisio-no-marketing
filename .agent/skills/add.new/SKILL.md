...
**MANDATORY GATE:** The `feature-about` schema must be loaded from `.agent/skills/add-doc-schemas/SKILL.md` BEFORE writing any doc.
...
> **Skill:** Apply `.agent/skills/add-id-convention/SKILL.md` for ID/branch format.
...
EXECUTE schema `feature-about` from `.agent/skills/add-doc-schemas/SKILL.md`.

**Apply cache technique:** Read → Preserve → Complement → Metadata (per `.agent/skills/add-doc-schemas/SKILL.md`)
...
Execute the validation gate from `.agent/skills/add-doc-schemas/SKILL.md` for schema `feature-about`.
...
The agent MUST NOT see this conversation — its job is to read only the doc and the schema. If the provider does not support subagent dispatch, apply `.agent/skills/add-doc-reviewer/SKILL.md` inline, explicitly forgetting the conversation.
