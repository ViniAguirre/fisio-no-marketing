...
- Command/skill/script path references — use `{{cmd:}}` / `{{skill:}}` per `.claude/skills/add-resource-path-convention/SKILL.md`
...
> **Convention:** Feature/hotfix/refactor/chore/docs IDs use `[NNNN][L]` (number first, letter suffix) per `.claude/skills/add-id-convention/SKILL.md`. `PRD` and `CHG` are separate namespaces with no letter suffix.
...
- **Refs** — doc→doc links MUST use `{{doc:<ID>}}` syntax per the ID Convention section. Command/skill refs use `{{cmd:}}` / `{{skill:}}` per `.claude/skills/add-resource-path-convention/SKILL.md`.
...
- `.claude/skills/add-token-efficiency/SKILL.md` — generic compression patterns for authoring commands/skills
- `.claude/skills/add-resource-path-convention/SKILL.md` — build-time resource refs (`{{cmd:}}` / `{{skill:}}`)
- `.claude/skills/add-claude-md-style/SKILL.md` — CLAUDE.md generation rules (separate pipeline)
