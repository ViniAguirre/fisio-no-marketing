...
**MANDATORY GATE:** The `feature-about` schema must be loaded from `.claude/skills/add-doc-schemas/SKILL.md` BEFORE writing any doc.
...
> **Skill:** Apply `.claude/skills/add-id-convention/SKILL.md` for ID/branch format.
...
EXECUTE schema `feature-about` from `.claude/skills/add-doc-schemas/SKILL.md`.

**Apply cache technique:** Read → Preserve → Complement → Metadata (per `.claude/skills/add-doc-schemas/SKILL.md`)
...
Execute the validation gate from `.claude/skills/add-doc-schemas/SKILL.md` for schema `feature-about`.
...
The agent MUST NOT see this conversation — its job is to read only the doc and the schema. If the provider does not support subagent dispatch, apply `.claude/skills/add-doc-reviewer/SKILL.md` inline, explicitly forgetting the conversation.
