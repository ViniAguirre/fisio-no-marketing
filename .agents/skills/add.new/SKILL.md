...
**MANDATORY GATE:** The `feature-about` schema must be loaded from `.agents/skills/add-doc-schemas/SKILL.md` BEFORE writing any doc.
...
> **Skill:** Apply `.agents/skills/add-id-convention/SKILL.md` for ID/branch format.
...
EXECUTE schema `feature-about` from `.agents/skills/add-doc-schemas/SKILL.md`.

**Apply cache technique:** Read → Preserve → Complement → Metadata (per `.agents/skills/add-doc-schemas/SKILL.md`)
...
Execute the validation gate from `.agents/skills/add-doc-schemas/SKILL.md` for schema `feature-about`.
...
The agent MUST NOT see this conversation — its job is to read only the doc and the schema. If the provider does not support subagent dispatch, apply `.agents/skills/add-doc-reviewer/SKILL.md` inline, explicitly forgetting the conversation.
