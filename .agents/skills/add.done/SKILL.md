...
> **Skill:** Apply `.agents/skills/add-id-convention/SKILL.md` for ID/branch format. Changelog IDs use `CHG[NNNN]` (no letter suffix — different namespace).
...
**Schema load (MANDATORY).** EXECUTE schema `changelog` from `.agents/skills/add-doc-schemas/SKILL.md`.
...
Execute the validation gate from `.agents/skills/add-doc-schemas/SKILL.md` for schema `changelog`.
...
1. Read `.agents/skills/add.pr/SKILL.md` as reference for PR creation flow
